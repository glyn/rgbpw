...
func NewOpsManagerClient(hostname, username, password string, logger lager.Logger) (*opsmanClient, error) {
	...
	token, err := getAuthorization(url, username, password, logger)
...
func (products products) CF() product {
	...
	panic("Can't find cf product in installation settings")
	return product{}
}
...
func (jobs jobs) UAA() job {
	...
	panic("Can't find uaa job in product settings")
	return job{}
}
...
func (properties properties) AdminCredentials() property {
	...
	panic("Can't find admin_credentials in uaa job")
	return property{}
}
...
func getAuthorization(opsManagerURL, username, password string, logger lager.Logger) (string, error) {
	...
	if err != nil {
		logger.Error("NewRequest failed:", err)
		return "", err
	}
	...
	if response.StatusCode == 200 {
		...
		if err != nil {
			logger.Error("NewDecoder failed:", err)
			return "", err
		}
